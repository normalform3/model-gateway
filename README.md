...
> 管理控制台通过统一账号密码入口提供平台管理员、团队管理员和开发者三类 RBAC；模型调用接口仍只使用虚拟 API Key 鉴权，二者互不替代。
...
控制台默认地址为 `http://localhost:5173`，Vite 会将 `/admin` 与 `/v1` 代理到 `http://localhost:8080`。未指定 profile 的本地启动和 `dev` profile 支持开发期账号规则，但不会在启动时修改数据库。新拉取仓库且需要初始化空本地数据库时，可运行 `bash tools/init-dev-console-credentials.sh`；它会将平台管理员初始化为 `admin / 123`，其他用户按名称生成 `@modelgate.local` 账号。生产环境必须显式使用非 `default` 的 profile 或设置 `MODELGATE_DEVELOPMENT_DEFAULT_CREDENTIALS_ENABLED=false`。初始化可重复执行的 Showcase 数据：
...
当前版本已实现 Mock 与 OpenAI-Compatible 接入、虚拟 API Key、团队 / 成员 / 项目额度、流式调用、异步计量和带 RBAC 的管理控制台；跨模型智能路由、熔断降级、全链路监控和 Provider 账单回查属于后续工作。详细计划见[开发路线图](docs/ROADMAP.md)。
...
- 控制台 access token 仅保存在浏览器内存；刷新令牌仅以 HttpOnly Cookie 传递，数据库只保存其哈希。
- 生产环境设置 `MODELGATE_CONSOLE_JWT_SECRET`（至少 32 字节）和 `MODELGATE_BOOTSTRAP_ADMIN_PASSWORD_HASH` 后，种子平台管理员才能登录；不得启用开发期固定凭据。
