...
public class TeamEntitlementRepository {
    ...
    private void grant(long teamId, List<String> models, long tokens, OffsetDateTime expires, String reference) {
        ...
        account("TEAM", teamId); jdbc.update("UPDATE quota_account SET available_tokens = available_tokens + ?, version = version + 1, updated_at = ? WHERE account_type = 'TEAM' AND owner_id = ?", tokens, now(), teamId); long account = require("SELECT id FROM quota_account WHERE account_type = 'TEAM' AND owner_id = ?", teamId); long balance = require("SELECT available_tokens FROM quota_account WHERE id = ?", account); jdbc.update("INSERT INTO quota_transaction(transaction_no, account_id, transaction_type, amount, balance_after, created_at) VALUES (?, ?, 'TEAM_GRANT', ?, ?, ?)", "qt-team-grant-" + reference, account, tokens, balance, now()); jdbc.update("UPDATE team SET status = 'ACTIVE' WHERE id = ?", teamId);
    }
    ...
    @Transactional public MemberAccessSnapshot grantMemberAccess(long teamId, long memberId, GrantMemberAccessRequest request) {
        ...
        account("TEAM", teamId); account("MEMBER", memberId); long from = require("SELECT id FROM quota_account WHERE account_type = 'TEAM' AND owner_id = ?", teamId); long to = require("SELECT id FROM quota_account WHERE account_type = 'MEMBER' AND owner_id = ?", memberId); Long available = nullable("SELECT available_tokens FROM quota_account WHERE id = ? FOR UPDATE", from); if (available == null || available < request.tokenAllocation()) throw new ModelGateException(ErrorCode.QUOTA_INSUFFICIENT, "The team does not have enough unallocated Token quota.");
